...
func TestHandleKeyPress_EnterInSplitPane(t *testing.T) {
	m := NewModel(context.Background(), "/tmp/wt", "test-repo", "code", session.NewManager(), nil, nil, 80, 24, nil, nil, session.ManagerConfig{})
...
func TestBuildHelpSections_SplitPaneActive(t *testing.T) {
	...
	m := NewModel(context.Background(), "/tmp/wt", "test-repo", "code", mgr, nil, nil, 80, 24, nil, nil, session.ManagerConfig{})
	m.splitPane.Toggle()
...
func TestHandleKeyPress_UpDownNavigatesSessionListInTmuxSplitRightFocus(t *testing.T) {
	...
	m := NewModel(context.Background(), "/tmp/wt", "test-repo", "", mgr, nil, worktrees, 80, 24, nil, nil, session.ManagerConfig{})
