...
func TestHandleKeyPress_EnterInSplitPane(t *testing.T) {
	m := NewModel(context.Background(), "/tmp/wt", "test-repo", "code", session.NewManager(), nil, nil, nil, 80, 24)
...
func TestBuildHelpSections_SplitPaneActive(t *testing.T) {
	...
	m := NewModel(context.Background(), "/tmp/wt", "test-repo", "code", mgr, nil, nil, nil, 80, 24)
	m.splitPane.Toggle()
...
func TestHandleKeyPress_UpDownNavigatesSessionListInTmuxSplitRightFocus(t *testing.T) {
	...
	m := NewModel(context.Background(), "/tmp/wt", "test-repo", "", mgr, nil, nil, worktrees, 80, 24)
