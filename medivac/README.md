...
Automated CI failure remediation system. Medivac scans GitHub Actions for failures, triages them using an LLM, tracks issues across runs, launches Claude agents to create fix PRs, and manages the full lifecycle through merge and verification.
...
For each failed workflow run, medivac makes a **single LLM call** containing:
...
Issues are identified by a stable signature (`{category}:{hash}:{file}`) that survives across runs. The tracker persists to `.medivac/issues.json`.
...
```bash
medivac scan --repo-root /path/to/repo --branch main --limit 5 -v
```
