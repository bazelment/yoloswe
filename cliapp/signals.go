...
//
// The returned cancel must be called to release resources (and to silence the
// signal goroutine) once the caller is done.
//
...
func notifyContext(parent context.Context, forceExit func()) (context.Context, context.CancelFunc) {
	...
	go func() {
		...
		case <-done:
			return
		case <-parent.Done():
			// Parent gave up before any signal arrived; nothing to do.
			return
		case <-sigCh:
			cancel()
		}
